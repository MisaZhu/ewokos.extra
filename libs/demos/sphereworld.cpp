...
vector<rsw::vec3> box_verts;
vector<rsw::ivec3> box_tris;
vector<rsw::vec2> box_tex;
...
GLFrame camera_frame(true, rsw::vec3(0, 0.3, 2.5));
...
int main(int argc, char** argv)
{
	...
	if (!load_texture2D("/data/media/textures/marble.tga", GL_LINEAR, GL_LINEAR, GL_REPEAT, false, false, NULL, NULL)) {
		printf("failed to load texture\n");
		return 0;
	}
	glBindTexture(GL_TEXTURE_2D, textures[1]);

	if (!load_texture2D("/data/media/textures/mars.tga", GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, false, false, NULL, NULL)) {
		printf("failed to load texture\n");
		return 0;
	}

	glBindTexture(GL_TEXTURE_2D, textures[2]);
	if (!load_texture2D("/data/media/textures/moon.tga", GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, false, false, NULL, NULL)) {
		printf("failed to load texture\n");
		return 0;
	}
	...
	vector<rsw::vec3> torus_verts;
	vector<rsw::vec2> torus_tex;
	vector<rsw::ivec3> torus_tris;
	vector<rsw::vec3> torus_draw_verts;
	vector<rsw::vec2> torus_draw_tex;
	float major = .3;
...
*/
	...
	vector<rsw::vec3> floor_verts;
	vector<rsw::vec3> floor_draw_verts;
	vector<rsw::vec2> floor_tex;
	vector<rsw::ivec3> floor_tris;
	make_plane(floor_verts, floor_tris, floor_tex, rsw::vec3(-FLOOR_SIZE/2.0, 0, FLOOR_SIZE/2.0), rsw::vec3(0, 0, -FLOOR_SIZE), rsw::vec3(FLOOR_SIZE, 0, 0), 1, 1, false);
	...
	sphere_vao.bind();
	vector<rsw::vec3> sphere_verts;
	vector<rsw::vec3> sphere_draw_verts;
	vector<rsw::vec2> sphere_tex;
	vector<rsw::ivec3> sphere_tris;
	make_sphere(sphere_verts, sphere_tris, sphere_tex, 0.1, 26, 13);

	vector<rsw::vec3> sphere_normals;
	...
	for (int i=0; i<NUM_SPHERES+1; ++i) {
		if (i)
			instance_pos.push_back(rsw::vec3(rsw::randf_range(-FLOOR_SIZE/2.0, FLOOR_SIZE/2.0), 0.4, rsw::randf_range(-FLOOR_SIZE/2.0, FLOOR_SIZE/2.0)));
		else
	...
	the_uniforms.Ka = rsw::vec3(0.1, 0.1, 0.1);
	the_uniforms.Kd = rsw::vec3(0.8, 0.8, 0.8);
	the_uniforms.Ks = rsw::vec3(1.0, 1.0, 1.0);
	the_uniforms.Shininess = 128.0f;

	SDL_SetRelativeMouseMode(SDL_TRUE);

	rsw::vec4 modulate_color(1, 1, 1, 0.75);

	rsw::mat4 scale_mat = rsw::scale_mat4(1.0f, -1.0f, 1.0f);
	rsw::mat4 translate_sphere = rsw::translation_mat4(0.8f, 0.4f, 0.0f);

	rsw::mat4 VP, torus_rot_mat, sphere_rot_mat;
	rsw::mat4 camera_mat;
	...
	while (1) {
		...
		if (rand() % 50 == 0) {
			...
			for (int i=1; i<NUM_SPHERES+1; ++i) {
				instance_pos[i] = rsw::vec3(rsw::randf_range(-FLOOR_SIZE/2.0, FLOOR_SIZE/2.0), 0.4, rsw::randf_range(-FLOOR_SIZE/2.0, FLOOR_SIZE/2.0));
			}
		}
		...
		rsw::load_rotation_mat4(sphere_rot_mat, rsw::vec3(0, 1, 0), -2*total_time*DEG_TO_RAD(60.0f));
		...
		rsw::load_rotation_mat4(torus_rot_mat, rsw::vec3(0, 1, 0), total_time*DEG_TO_RAD(60.0f));
...
void texture_replace_instanced_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	pgl_vec4* v_attribs = vertex_attribs;

	((pgl_vec2*)vs_output)[0] = *(pgl_vec2*)&v_attribs[2]; //tex_coords

	pgl_vec4 tmp = v_attribs[3];
	builtins->gl_Position = mult_m4_v4(*(pgl_mat4*)uniforms, make_v4(tmp.x, tmp.y, tmp.z, 0.0f));

}
void texture_replace_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	pgl_vec2 tex_coords = *(pgl_vec2*)fs_input;
...
void texture_replace_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	((pgl_vec2*)vs_output)[0] = *(pgl_vec2*)&vertex_attribs[2]; //tex_coords
...
void texture_replace_modulate_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	pgl_vec2 tex_coords = *(pgl_vec2*)fs_input;
	GLuint tex = ((My_Uniforms*)uniforms)->tex0;
	rsw::vec4 rsw_modulate = ((My_Uniforms*)uniforms)->modulate;
	pgl_vec4 modulate = make_v4(rsw_modulate.x, rsw_modulate.y, rsw_modulate.z, rsw_modulate.w);

	pgl_vec4 tmp = texture2D(tex, tex_coords.x, tex_coords.y);
	builtins->gl_FragColor = mult_v4s(scale_v4(modulate, tmp.w), tmp);
}
...
void texture_ADS_instanced_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	pgl_vec3 light_dir = make_v3(0, 1, 0);
	...
	pgl_vec4 tmp = make_v4(vert_attribs[3].x, vert_attribs[3].y, vert_attribs[3].z, 0.0f);
	builtins->gl_Position = mult_m4_v4(*(pgl_mat4*)&u->mvp_mat, add_v4s(vert_attribs[ATTR_VERTEX], tmp));
}

void texture_ADS_fs(float* fs_input, Shader_Builtins* builtins, void* uniforms)
{
	pgl_vec2 tex_coords = make_v2(fs_input[0], fs_input[1]);
...
void texture_ADS_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	builtins->gl_Position = mult_m4_v4(*(pgl_mat4*)&u->mvp_mat, vert_attribs[ATTR_VERTEX]);
...
bool handle_events()
{
	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_KEYDOWN:
			...
			return true;
		} else if (keysym.sym == keyvalues[PROVOKING]) {
		...
		} else if (keysym.sym == keyvalues[INTERPOLATION]) {
		...
		} else if (keysym.sym == keyvalues[POLYGONMODE]) {
