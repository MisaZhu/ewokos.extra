...
bool handle_events(GLFrame& camera_frame);
void cleanup();
void setup_context();
void setup_gl_data();
...
int main(int argc, char** argv)
{
	...
	*/
	...
	while (1) {
		...
		// Flip vertically (OpenGL Y-up vs SDL Y-down)
...
void reflection_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	vec3 c = coords.xyz().norm();
	// Note: Y flip removed since we now flip the entire framebuffer
	((vec3*)vs_output)[0] = c;
...
void skybox_vs(float* vs_output, pgl_vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms)
{
	...
	vec3 s = v_attrs[0].xyz().norm();
	s.y = -s.y; // flip Y for cubemap
	((vec3*)vs_output)[0] = s;
...
void setup_context()
{
	...
	window = SDL_CreateWindow("Cubemap", 100, 100, WIDTH, HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	if (!window) {
	...
	SDL_tex = SDL_CreateTexture(ren, PIX_FORMAT, SDL_TEXTUREACCESS_STREAMING, WIDTH, HEIGHT);
