...
static uint64_t aac_perf_now_us(void)
{
    return (uint64_t)av_gettime_relative();
}

static void aac_perf_maybe_report(AACContext *ac)
{
    uint64_t now_us;
    double wall_ms;
    double fps;

    now_us = aac_perf_now_us();
    if (ac->perf_report_start_us == 0) {
        ac->perf_report_start_us = now_us;
        ac->perf_last_report_us = now_us;
        return;
    }

    if (now_us - ac->perf_last_report_us < 3000000ULL)
        return;

    wall_ms = (double)(now_us - ac->perf_report_start_us) / 1000.0;
    fps = wall_ms > 0.0 ? ((double)ac->perf_frames * 1000.0 / wall_ms) : 0.0;

    av_log(ac->avctx, AV_LOG_ERROR,
           "aacperf: wall=%.1fms frames=%llu fps=%.2f frame=%.1fms "
           "ics=%.1fms tns_parse=%.1fms spectrum=%.1fms spec2samp=%.1fms "
           "tns_apply=%.1fms imdct=%.1fms ltp=%.1fms\n",
           wall_ms,
           (unsigned long long)ac->perf_frames,
           fps,
           (double)ac->perf_frame_us / 1000.0,
           (double)ac->perf_decode_ics_us / 1000.0,
           (double)ac->perf_tns_parse_us / 1000.0,
           (double)ac->perf_spectrum_us / 1000.0,
           (double)ac->perf_spectral_to_sample_us / 1000.0,
           (double)ac->perf_tns_apply_us / 1000.0,
           (double)ac->perf_imdct_us / 1000.0,
           (double)ac->perf_ltp_us / 1000.0);
    ac->perf_last_report_us = now_us;
}
...
static void flush(AVCodecContext *avctx)
{
    ...
    ac->perf_report_start_us = 0;
    ac->perf_last_report_us = 0;
    ac->perf_frames = 0;
    ac->perf_frame_us = 0;
    ac->perf_decode_ics_us = 0;
    ac->perf_tns_parse_us = 0;
    ac->perf_spectrum_us = 0;
    ac->perf_spectral_to_sample_us = 0;
    ac->perf_tns_apply_us = 0;
    ac->perf_imdct_us = 0;
    ac->perf_ltp_us = 0;
}
                      ...
                      GetBitContext *gb, int common_window, int scale_flag)
{
    ...
    int ret;
    uint64_t begin_us = aac_perf_now_us();
    uint64_t stage_us;
    ...
    if (!scale_flag) {
        ...
        if (tns->present && !er_syntax) {
            stage_us = aac_perf_now_us();
            ret = decode_tns(ac, tns, gb, ics);
            ac->perf_tns_parse_us += aac_perf_now_us() - stage_us;
            if (ret < 0)
                goto fail;
        }
    ...
    ac->perf_spectrum_us += aac_perf_now_us() - stage_us;
    if (ret < 0)
        goto fail;
    ...
    ac->perf_decode_ics_us += aac_perf_now_us() - begin_us;
    return 0;
fail:
    tns->present = 0;
    ac->perf_decode_ics_us += aac_perf_now_us() - begin_us;
...
static void spectral_to_sample(AACContext *ac, int samples)
{
        ...
        for (i = 0; i < MAX_ELEM_ID; i++) {
            ...
            if (che && che->present) {
                ...
                if (ac->oc[1].m4ac.object_type == AOT_AAC_LTP) {
                    if (che->ch[0].ics.predictor_present) {
                        if (che->ch[0].ics.ltp.present)
                        {
                            ...
                            ac->perf_ltp_us += aac_perf_now_us() - stage_us;
                        }
                ...
                if (che->ch[0].tns.present)
                {
                    ...
                    ac->perf_tns_apply_us += aac_perf_now_us() - stage_us;
                }
                ...
                if (type != TYPE_CCE || che->coup.coupling_point == AFTER_IMDCT) {
                    ...
                    ac->perf_imdct_us += aac_perf_now_us() - stage_us;
                    if (ac->oc[1].m4ac.object_type == AOT_AAC_LTP)
    ...
    ac->perf_spectral_to_sample_us += aac_perf_now_us() - begin_us;
}
                                ...
                                const AVPacket *avpkt)
{
    ...
    *got_frame_ptr = !!samples;
    ...
    ac->perf_frames++;
    ac->perf_frame_us += aac_perf_now_us() - begin_us;
    aac_perf_maybe_report(ac);
    return 0;
fail:
    ac->perf_frame_us += aac_perf_now_us() - begin_us;
    pop_output_configuration(ac);
