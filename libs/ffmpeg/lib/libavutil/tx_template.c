                                               ...
                                               const void *scale)
{
    ...
    /* PFA needs a non-trivial coprime decomposition. Pure powers of two
     * must stay on the dedicated radix-2/split-radix path. */
    if (!(len & (len - 1)))
                                    ...
                                    ptrdiff_t stride)
{
    ...
    const int len4 = s->len >> 2;
    const int *sub_map = s->map;
    static int logged_enter_1024;
    static int logged_fft_done_1024;

    stride /= sizeof(*src);
    in1 = src;
    in2 = src + ((len2*2) - 1) * stride;

    if (ARCH_AARCH64 && s->len == 1024 && !logged_enter_1024) {
        av_log(NULL, AV_LOG_ERROR,
               "ffmpeg tx: mdct_inv opt enter len=%d preshuffle=%d\n",
               s->len, !!(s->sub[0].flags & FF_TX_PRESHUFFLE));
        logged_enter_1024 = 1;
    }
    ...
    if (ARCH_AARCH64 && s->len == 1024 && !logged_fft_done_1024) {
        av_log(NULL, AV_LOG_ERROR, "ffmpeg tx: mdct_inv opt fft-done len=%d\n",
               s->len);
        logged_fft_done_1024 = 1;
    }
