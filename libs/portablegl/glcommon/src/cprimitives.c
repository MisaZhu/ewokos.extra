...
void make_box2(cvector_vec3* verts, cvector_ivec3* tris, cvector_vec2* tex, float dimX, float dimY, float dimZ, int plane, ivec3 seg, vec3 origin)
{
    ...
    vec3 orig_x = add_v3s(origin, x_vec);
    vec3 orig_y = add_v3s(origin, y_vec);
    vec3 orig_xy = add_v3s(orig_x, y_vec);
    vec3 orig_xyz = add_v3s(orig_xy, z_vec);

    vec3 orig_z = add_v3s(origin, z_vec);
    vec3 orig_zx = add_v3s(orig_z, x_vec);
    vec3 orig_zy = add_v3s(orig_z, y_vec);
    ...
    if (!plane) {
        ...
        for (int i=tri_start; i<tris->size; i++) {
            //TODO
            //tris->a[i] = add_iv3s(tris->a[i], make_iv3(vert_start, vert_start, vert_start));
        ...
        //front and back
        make_plane2(verts, tris, tex, orig_x, y_div_segy, neg_v3(x_div_segx), seg.y, seg.x, TRUE);
        ...
        make_plane2(verts, tris, tex, orig_zx, y_div_segy, neg_v3(z_div_segz), seg.y, seg.z, TRUE);
        //top and bottom
        make_plane2(verts, tris, tex, origin, z_div_segz, x_div_segx, seg.z, seg.x, TRUE);
        make_plane2(verts, tris, tex, orig_zy, neg_v3(z_div_segz), x_div_segx, seg.z, seg.x, TRUE);
    }
}
...
void make_cylindrical(cvector_vec3* verts, cvector_ivec3* tris, cvector_vec2* tex, float radius, float height, size_t slices, size_t stacks, float top_radius)
{
    ...
    cvec_push_vec3(verts, make_v3(0, 0, 0));

    for (i = 0; i <= stacks; i++) {
        cur_radius = i/stacks * top_radius + (1.0 - i/stacks) * radius;
        for (j = 0; j < slices; j++) {
            cvec_push_vec3(verts, make_v3(radius*cos(j*theta), radius*sin(j*theta), i*stack_height));
        }
    }

    cvec_push_vec3(verts, make_v3(0, 0, height));
...
//I'll figure it out for increasing later.
    for (i = slices; i > 0; i--) {
        cvec_push_ivec3(tris, make_iv3(0, (((i+1)%(slices+1)==0) ? 1 : i+1), i));
...
void make_plane2(cvector_vec3* verts, cvector_ivec3* tris, cvector_vec2* tex, vec3 corner, vec3 v1, vec3 v2, size_t dimV1, size_t dimV2, int tile)
{
    ...
    for (i = 0; i <= dimV2; i++) {
        for (int j=0; j <= dimV1; j++) {
            SET_V3(tmp_vec, corner.x + v2.x*i + v1.x*j, corner.y + v2.y*i + v1.y*j, corner.z + v2.z*i + v1.z*j);
            //tmp_vec = add_v3s(add_v3s(corner, scale_v3(v2, i)), scale_v3(v1, j));
    ...
    for (i = 0; i < dimV1*dimV2; i++) {
        ...
        cvec_push_ivec3(tris, make_iv3(i+j, i+j+dimV1+1, i+j+dimV1+2));
        cvec_push_ivec3(tris, make_iv3(i+j, i+j+dimV1+2, i+j+1));

//		cvec_push_ivec3(tris, make_iv3(i+j, i+j+1, i+j+1+dimV2));
//		cvec_push_ivec3(tris, make_iv3(i+j+1+dimV2, i+j+1, i+j+dimV2+2));
...
void make_sphere(cvector_vec3* verts, cvector_ivec3* tris, cvector_vec2* tex, float radius, size_t slices, size_t stacks)
{
    ...
    load_rotation_m3(rotdown, make_v3(0, 1, 0), down);
    load_rotation_m3(rotaround, make_v3(0, 0, 1), around);
    ...
    for (int i=1; i<stacks; ++i) {
        ...
        point = mult_m3_v3(rotdown, point);
        cvec_push_vec3(verts, point);
        tmp = point;

        for (int j=1; j<slices; ++j) {
            point = mult_m3_v3(rotaround, point);
            cvec_push_vec3(verts, point);
        }
        point = tmp; //set back to y (copying probably quicker than rotating again)
    }

    cvec_push_vec3(verts, make_v3(0, 0, -radius));
    ...
    for (int i=1 ; i<stacks-1 ; ++i) {
        for (int j=0; j<slices; ++j) {
            ...
            corner = i*slices+1 + j;
            if (j != slices-1) {
                cvec_push_ivec3(tris, make_iv3(corner, corner+1, corner-slices));
...
void make_torus(cvector_vec3* verts, cvector_ivec3* tris, cvector_vec2* tex, float major_r, float minor_r, size_t major_slices, size_t minor_slices)
{
    ...
    for (i=0; i<major_slices; ++i) {
        ...
        for (j=0; j<minor_slices; ++j) {
            ...
            cvec_push_vec3(verts, make_v3(x0 * r, y0 * r, z));
        }
    }
    ...
    for (i=0; i<major_slices; ++i) {
        ...
        if (i != major_slices-1) {
            for (j=0; j<minor_slices; ++j) {
                if (j != minor_slices-1) {
                    cvec_push_ivec3(tris, make_iv3(s+j, s+j+1+minor_slices, s+j+1));
                    cvec_push_ivec3(tris, make_iv3(s+j, s+j+minor_slices, s+j+1+minor_slices));
                } else {
            ...
            for (j=0; j<minor_slices; ++j) {
                if (j != minor_slices-1) {
                    cvec_push_ivec3(tris, make_iv3(s+j, j+1, s+j+1));
                    cvec_push_ivec3(tris, make_iv3(s+j, j, j+1));
                } else {
...
void make_tetrahedron(cvector_vec3* verts, cvector_ivec3* tris)
{
    ...
    cvec_push_vec3(verts, make_v3(1, 1, 1));
    cvec_push_vec3(verts, make_v3(1, -1, -1));
    cvec_push_vec3(verts, make_v3(-1, 1, -1));
    cvec_push_vec3(verts, make_v3(-1, -1, 1));

    cvec_push_ivec3(tris, make_iv3(0, 1, 2));
    cvec_push_ivec3(tris, make_iv3(0, 2, 3));
    cvec_push_ivec3(tris, make_iv3(0, 3, 1));
    cvec_push_ivec3(tris, make_iv3(1, 3, 2));
...
void make_octahedron(cvector_vec3* verts, cvector_ivec3* tris)
{
    ...
    cvec_push_vec3(verts, make_v3(1, 0, 0));
    cvec_push_vec3(verts, make_v3(0, 0, -1));
    cvec_push_vec3(verts, make_v3(-1, 0, 0));
    cvec_push_vec3(verts, make_v3(0, 0, 1));
    cvec_push_vec3(verts, make_v3(0, 1, 0));
    cvec_push_vec3(verts, make_v3(0, -1, 0));

    cvec_push_ivec3(tris, make_iv3(0, 1, 4));
    cvec_push_ivec3(tris, make_iv3(1, 2, 4));
    cvec_push_ivec3(tris, make_iv3(2, 3, 4));
    cvec_push_ivec3(tris, make_iv3(3, 0, 4));

    cvec_push_ivec3(tris, make_iv3(1, 0, 5));
    cvec_push_ivec3(tris, make_iv3(2, 1, 5));
    cvec_push_ivec3(tris, make_iv3(3, 2, 5));
    cvec_push_ivec3(tris, make_iv3(0, 3, 5));
...
void make_dodecahedron(cvector_vec3* verts, cvector_ivec3* tris)
{
    ...
    cvec_push_vec3(verts, make_v3(1, 1, 1));
    cvec_push_vec3(verts, make_v3(1/phi, phi, 0));
    cvec_push_vec3(verts, make_v3(-1, 1, 1));
    cvec_push_vec3(verts, make_v3(0, 1/phi, phi));
    cvec_push_vec3(verts, make_v3(-1/phi, phi, 0));
    cvec_push_vec3(verts, make_v3(1, 1, 1));
    cvec_push_vec3(verts, make_v3(phi, 0, 1/phi));
    cvec_push_vec3(verts, make_v3(1, 1, -1));
    cvec_push_vec3(verts, make_v3(1/phi, phi, 0));
    cvec_push_vec3(verts, make_v3(phi, 0, -1/phi));
    ...
    cvec_push_ivec3(tris, make_iv3(0, 1, 2));
    cvec_push_ivec3(tris, make_iv3(2, 3, 0));
    cvec_push_ivec3(tris, make_iv3(1, 4, 2));
    cvec_push_ivec3(tris, make_iv3(5, 6, 7));
    cvec_push_ivec3(tris, make_iv3(7, 8, 5));
    cvec_push_ivec3(tris, make_iv3(6, 9, 7));
    cvec_push_ivec3(tris, make_iv3(10, 11, 12));
    cvec_push_ivec3(tris, make_iv3(12, 13, 10));
    cvec_push_ivec3(tris, make_iv3(11, 14, 12));
    cvec_push_ivec3(tris, make_iv3(15, 16, 17));
    cvec_push_ivec3(tris, make_iv3(17, 18, 15));
    cvec_push_ivec3(tris, make_iv3(16, 19, 17));
    cvec_push_ivec3(tris, make_iv3(20, 21, 22));
    cvec_push_ivec3(tris, make_iv3(22, 23, 20));
    cvec_push_ivec3(tris, make_iv3(21, 24, 22));
    cvec_push_ivec3(tris, make_iv3(25, 26, 27));
    cvec_push_ivec3(tris, make_iv3(27, 28, 25));
    cvec_push_ivec3(tris, make_iv3(26, 29, 27));
    cvec_push_ivec3(tris, make_iv3(30, 31, 32));
    cvec_push_ivec3(tris, make_iv3(32, 33, 30));
    cvec_push_ivec3(tris, make_iv3(31, 34, 32));
    cvec_push_ivec3(tris, make_iv3(35, 36, 37));
    cvec_push_ivec3(tris, make_iv3(37, 38, 35));
    cvec_push_ivec3(tris, make_iv3(36, 39, 37));
    cvec_push_ivec3(tris, make_iv3(40, 41, 42));
    cvec_push_ivec3(tris, make_iv3(42, 43, 40));
    cvec_push_ivec3(tris, make_iv3(41, 44, 42));
    cvec_push_ivec3(tris, make_iv3(45, 46, 47));
    cvec_push_ivec3(tris, make_iv3(47, 48, 45));
    cvec_push_ivec3(tris, make_iv3(46, 49, 47));
    cvec_push_ivec3(tris, make_iv3(50, 51, 52));
    cvec_push_ivec3(tris, make_iv3(52, 53, 50));
    cvec_push_ivec3(tris, make_iv3(51, 54, 52));
    cvec_push_ivec3(tris, make_iv3(55, 56, 57));
    cvec_push_ivec3(tris, make_iv3(57, 58, 55));
    cvec_push_ivec3(tris, make_iv3(56, 59, 57));
...
void make_icosahedron(cvector_vec3* verts, cvector_ivec3* tris)
{
    ...
    cvec_push_vec3(verts, make_v3(0, 1, phi));
    cvec_push_vec3(verts, make_v3(0, -1, phi));
    cvec_push_vec3(verts, make_v3(0, -1, -phi));
    cvec_push_vec3(verts, make_v3(0, 1, -phi));

    cvec_push_vec3(verts, make_v3(1, phi, 0));
    cvec_push_vec3(verts, make_v3(-1, phi, 0));
    cvec_push_vec3(verts, make_v3(-1, -phi, 0));
    cvec_push_vec3(verts, make_v3(1, -phi, 0));

    cvec_push_vec3(verts, make_v3(phi, 0, 1));
    cvec_push_vec3(verts, make_v3(phi, 0, -1));
    cvec_push_vec3(verts, make_v3(-phi, 0, -1));
    cvec_push_vec3(verts, make_v3(-phi, 0, 1));

    // "top" centered at vertex 0
    cvec_push_ivec3(tris, make_iv3(0, 11, 1));
    cvec_push_ivec3(tris, make_iv3(0, 1, 8));
    cvec_push_ivec3(tris, make_iv3(0, 8, 4));
    cvec_push_ivec3(tris, make_iv3(0, 4, 5));
    cvec_push_ivec3(tris, make_iv3(0, 5, 11));

    // adjacent triangles to top
    cvec_push_ivec3(tris, make_iv3(1, 11, 6));
    cvec_push_ivec3(tris, make_iv3(1, 7, 8));
    cvec_push_ivec3(tris, make_iv3(8, 9, 4));
    cvec_push_ivec3(tris, make_iv3(5, 4, 3));
    cvec_push_ivec3(tris, make_iv3(11, 5, 10));

    //adjacent triangles to bottom
    cvec_push_ivec3(tris, make_iv3(6, 7, 1));
    cvec_push_ivec3(tris, make_iv3(7, 9, 8));
    cvec_push_ivec3(tris, make_iv3(9, 3, 4));
    cvec_push_ivec3(tris, make_iv3(3, 10, 5));
    cvec_push_ivec3(tris, make_iv3(10, 6, 11));
    
    // "bottom" centered at v 2
    cvec_push_ivec3(tris, make_iv3(2, 7, 6));
    cvec_push_ivec3(tris, make_iv3(2, 9, 7));
    cvec_push_ivec3(tris, make_iv3(2, 3, 9));
    cvec_push_ivec3(tris, make_iv3(2, 10, 3));
    cvec_push_ivec3(tris, make_iv3(2, 6, 10));
