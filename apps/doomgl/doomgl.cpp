...
static TexturedUniforms g_uniforms;
static MeshBatch g_wall_batches[5];
static MeshBatch g_floor_batch;
static MeshBatch g_ceiling_batch;
static GLuint g_enemy_textures[2][4];
static GLuint g_enemy_vao = 0;
static GLuint g_enemy_vbo = 0;
...
static void textured_vp(float* vs_output, vec4* vertex_attribs, Shader_Builtins* builtins, void* uniforms) {
	TexturedUniforms* u = (TexturedUniforms*)uniforms;
	...
	vs_output[2] = vertex_attribs[2].x;
}

static void textured_fp(float* fs_input, Shader_Builtins* builtins, void* uniforms) {
	TexturedUniforms* u = (TexturedUniforms*)uniforms;
	vec4 color = texture2D(u->tex0, fs_input[0], fs_input[1]);
	if (color.w <= 0.05f) {
...
static GLuint upload_texture_from_image(const Image& img) {
	GLuint texture = 0;
	glGenTextures(1, &texture);
	glBindTexture(GL_TEXTURE_2D, texture);

	const int pixel_count = img.w * img.h;
	std::vector<uint8_t> rgba((size_t)pixel_count * 4u);
	for (int i = 0; i < pixel_count; ++i) {
		uint32_t argb = img.pixels[(size_t)i];
		rgba[(size_t)i * 4u + 0u] = (uint8_t)((argb >> 16) & 0xff);
		rgba[(size_t)i * 4u + 1u] = (uint8_t)((argb >> 8) & 0xff);
		rgba[(size_t)i * 4u + 2u] = (uint8_t)(argb & 0xff);
		rgba[(size_t)i * 4u + 3u] = (uint8_t)((argb >> 24) & 0xff);
	}

	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img.w, img.h, 0, GL_RGBA, GL_UNSIGNED_BYTE, &rgba[0]);
...
static void init_batch(MeshBatch* batch) {
...
static void upload_batch(MeshBatch* batch) {
	batch->vertex_count = (int)batch->vertices.size();
	if (batch->vertex_count <= 0) return;

	glBindVertexArray(batch->vao);
	glBindBuffer(GL_ARRAY_BUFFER, batch->vbo);
	glBufferData(GL_ARRAY_BUFFER, batch->vertex_count * (int)sizeof(Vertex), &batch->vertices[0], GL_STATIC_DRAW);
...
static void append_vertex(MeshBatch& batch, float x, float y, float z, float u, float v, float shade) {
	Vertex vert;
	vert.x = x;
	vert.y = y;
	vert.z = z;
...
static void append_quad(MeshBatch& batch,
	float ax, float ay, float az,
	float bx, float by, float bz,
	float cx, float cy, float cz,
	float dx, float dy, float dz,
	float shade) {
	append_vertex(batch, ax, ay, az, 0.0f, 0.0f, shade);
	append_vertex(batch, bx, by, bz, 1.0f, 0.0f, shade);
	append_vertex(batch, cx, cy, cz, 1.0f, 1.0f, shade);
	append_vertex(batch, ax, ay, az, 0.0f, 0.0f, shade);
	append_vertex(batch, cx, cy, cz, 1.0f, 1.0f, shade);
	append_vertex(batch, dx, dy, dz, 0.0f, 1.0f, shade);
}

static void init_textures(void) {
	for (int i = 1; i <= 4; ++i) {
		g_wall_batches[i].texture = upload_texture_from_image(g_wall_textures[i]);
	}
	g_floor_batch.texture = upload_texture_from_image(g_floor_texture);
	g_ceiling_batch.texture = upload_texture_from_image(g_ceiling_texture);
	for (int archetype = 0; archetype < 2; ++archetype) {
		for (int frame = 0; frame < 4; ++frame) {
			g_enemy_textures[archetype][frame] = upload_texture_from_image(g_enemy_sprites[archetype][frame]);
		}
	}
}

static void build_world_mesh(void) {
	for (int i = 1; i <= 4; ++i) g_wall_batches[i].vertices.clear();
	g_floor_batch.vertices.clear();
	g_ceiling_batch.vertices.clear();

	for (int y = 0; y < MAP_H; ++y) {
		for (int x = 0; x < MAP_W; ++x) {
			if (is_wall_tile(x, y)) {
				int tex = wall_texture_index(x, y);
				if (!is_wall_tile(x, y - 1)) {
					append_quad(g_wall_batches[tex],
						(float)x, 0.0f, (float)y,
						(float)x + 1.0f, 0.0f, (float)y,
						(float)x + 1.0f, 1.0f, (float)y,
						(float)x, 1.0f, (float)y,
						0.92f);
				}
				if (!is_wall_tile(x, y + 1)) {
					append_quad(g_wall_batches[tex],
						(float)x + 1.0f, 0.0f, (float)y + 1.0f,
						(float)x, 0.0f, (float)y + 1.0f,
						(float)x, 1.0f, (float)y + 1.0f,
						(float)x + 1.0f, 1.0f, (float)y + 1.0f,
						0.92f);
				}
				...
				append_quad(g_floor_batch,
					(float)x, 0.0f, (float)y + 1.0f,
					(float)x + 1.0f, 0.0f, (float)y + 1.0f,
					(float)x + 1.0f, 0.0f, (float)y,
					(float)x, 0.0f, (float)y,
					0.82f);
				append_quad(g_ceiling_batch,
					(float)x, 1.0f, (float)y,
					(float)x + 1.0f, 1.0f, (float)y,
					(float)x + 1.0f, 1.0f, (float)y + 1.0f,
					(float)x, 1.0f, (float)y + 1.0f,
	...
	for (int i = 1; i <= 4; ++i) upload_batch(&g_wall_batches[i]);
	upload_batch(&g_floor_batch);
	upload_batch(&g_ceiling_batch);
}

static void init_gl(void) {
	GLenum interpolation[3] = { PGL_SMOOTH, PGL_SMOOTH, PGL_SMOOTH };
	g_program = CreateProgram(textured_vp, textured_fp, 3, interpolation, GL_TRUE);
	glUseProgram(g_program);
	SetUniform(&g_uniforms);

	for (int i = 1; i <= 4; ++i) init_batch(&g_wall_batches[i]);
	init_batch(&g_floor_batch);
	init_batch(&g_ceiling_batch);
	glGenVertexArrays(1, &g_enemy_vao);
	glGenBuffers(1, &g_enemy_vbo);
...
static void render_batch(const MeshBatch& batch, const mat4 vp) {
	if (batch.vertex_count <= 0) return;
	memcpy(g_uniforms.mvp_mat, vp, sizeof(mat4));
	g_uniforms.tex0 = batch.texture;
	glBindTexture(GL_TEXTURE_2D, batch.texture);
	glBindVertexArray(batch.vao);
	glDrawArrays(GL_TRIANGLES, 0, batch.vertex_count);
}

static void render_enemies_gl(const mat4 vp) {
	float right_x = -sinf(g_player_angle);
	float right_z = cosf(g_player_angle);
	...
	for (size_t i = 0; i < order.size(); ++i) {
		...
		for (size_t j = i + 1; j < order.size(); ++j) {
			if (order[j].distance_sq > order[best].distance_sq) best = j;
		}
		if (best != i) {
	...
	glBindVertexArray(g_enemy_vao);
	glBindBuffer(GL_ARRAY_BUFFER, g_enemy_vbo);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
	...
	for (size_t i = 0; i < order.size(); ++i) {
		const Enemy& enemy = g_enemies[(size_t)order[i].index];
		float dist = sqrtf(order[i].distance_sq);
		...
		float width = enemy.dead ? 0.62f : 0.72f;
		float height = enemy.dead ? 0.28f : 0.9f;
		float center_y = enemy.dead ? 0.15f : 0.45f;
		float half_w = width * 0.5f;
		float shade = 1.0f / (1.0f + dist * 0.18f);

		Vertex quad[6];
		quad[0] = { enemy.x - right_x * half_w, center_y - height * 0.5f, enemy.y - right_z * half_w, 0.0f, 0.0f, shade };
		quad[1] = { enemy.x + right_x * half_w, center_y - height * 0.5f, enemy.y + right_z * half_w, 1.0f, 0.0f, shade };
		quad[2] = { enemy.x + right_x * half_w, center_y + height * 0.5f, enemy.y + right_z * half_w, 1.0f, 1.0f, shade };
		quad[3] = quad[0];
		quad[4] = quad[2];
		quad[5] = { enemy.x - right_x * half_w, center_y + height * 0.5f, enemy.y - right_z * half_w, 0.0f, 1.0f, shade };

		memcpy(g_uniforms.mvp_mat, vp, sizeof(mat4));
		g_uniforms.tex0 = g_enemy_textures[enemy.archetype][frame];
		glBindTexture(GL_TEXTURE_2D, g_uniforms.tex0);
		glBufferData(GL_ARRAY_BUFFER, sizeof(quad), quad, GL_DYNAMIC_DRAW);
		glDrawArrays(GL_TRIANGLES, 0, 6);
	}
}

static void render_world_gl(void) {
	...
	mat4 proj_mat;
	mat4 vp_mat;
	vec3 eye = make_v3(g_player_x, 0.5f, g_player_y);
	vec3 center = make_v3(g_player_x + cosf(g_player_angle), 0.5f, g_player_y + sinf(g_player_angle));
	vec3 up = make_v3(0.0f, 1.0f, 0.0f);
	lookAt(view_mat, eye, center, up);
	make_perspective_m4(proj_mat, DEG_TO_RAD(68.0f), win_width / (float)win_height, 0.05f, 40.0f);
	mult_m4_m4(vp_mat, proj_mat, view_mat);

	render_batch(g_floor_batch, vp_mat);
	render_batch(g_ceiling_batch, vp_mat);
	for (int i = 1; i <= 4; ++i) render_batch(g_wall_batches[i], vp_mat);
	render_enemies_gl(vp_mat);
}
