...
void createMaps(MAP maps[], char dir[]){
	...
	char filePath[255];
	strcpy(filePath, dir);
	strcat(filePath, "res/levels.csv");
	...
	if(file != NULL){
		...
		printf("ERROR: Unable to read CSV File \"res/levels.csv\"");
	}
	fclose(file);
}
...
void initializeShapesRect(SDL_Rect arrayRects[], char dir[]){
	...
	char filePath[255];
	strcpy(filePath, dir);
	strcat(filePath, "res/game_hitbox.csv");
	FILE * file = fopen(filePath, "r");
	char line[256];
	if(file != NULL){
		...
		printf("ERROR: Unable to read CSV File \"res/game_hitbox.csv\"");
