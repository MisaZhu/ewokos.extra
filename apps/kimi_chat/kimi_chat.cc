...
class KimiChatWindow : public WidgetWin {
	...
	static void* requestThread(void* arg) {
		...
		kimi_client_message_t messages[1 + KIMI_CHAT_MAX_HISTORY + 1];
		int message_count = 0;
		PendingOutcome* outcome = new PendingOutcome();
		kimi_client_result_t result;
		int rc;
		...
		rc = kimi_client_chat(job->api_key.c_str(),
				job->model.c_str(),
		...
		kimi_client_result_clear(&result);
