...
//TODO doesn't handle boundaries correctly, unless sharp_angle == 0 then it just calculates face_normals
void compute_normals(vector<vec3>&verts, vector<ivec3>& t, half_edge_data* he_data_in, float sharp_angle, vector<vec3>&normals)
{
	...
	for (int i=0; i<t.size(); ++i) {
		...
		face_normals.push_back(tmp.norm());
	}
		...
		for (int i=0; i<t.size(); ++i) {
			...
			for (int j=0; j<3; ++j) {
				...
				while (edge2 != edge) {
					...
					//same face normals
					if (rsw::eql_epsilon(v1, v2, 0.00001f)) {
...
// 						}
						...
						std::cout<<v1<<"\t"<<v2<<"\t"<<v1.len()<<"\t"<<v2.len()<<"\t"<<angle<<"\t"<<sharp_angle<<"\n\n";
					}
...
// 					int k;
// 					for (k=0; k<tmp_normals.size(); ++k) {
// 						if (rsw::eql_epsilon(tmp_normals[k], v2, 0.0001f))
// 							break;
// 					}
// 					if (k == tmp_normals.size()) {
// 						tmp_normals.push_back(v2);
