...
int file_open_read_c_array(const char* filename, const char* mode, c_array* out)
{
	FILE *file = fopen(filename, mode);
	if (!file)
		return 0;

	return file_read_c_array(file, out);
}
...
int file_read_c_array(FILE* file, c_array* out)
{
	byte* data;
	long size;
	out->data = NULL;
