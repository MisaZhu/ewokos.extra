...
static int frame_is_ready_for_output(const AVCodecContext *avctx, const AVFrame *frame)
{
    ...
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO)
        return av_channel_layout_check(&frame->ch_layout) && frame->sample_rate > 0;
...
static int decode_simple_receive_frame(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    while (!frame_is_ready_for_output(avctx, frame)) {
        if (discarded_samples > avctx->max_samples)
            return AVERROR(EAGAIN);
        ret = decode_simple_internal(avctx, frame, &discarded_samples);
        if (ret < 0)
            return ret;
        if (frame->buf[0] && !frame_is_ready_for_output(avctx, frame))
            av_frame_unref(frame);
        if (!frame_is_ready_for_output(avctx, frame) &&
            !avci->in_pkt->data && !avci->draining)
            return AVERROR(EAGAIN);
    }
...
int attribute_align_arg avcodec_send_packet(AVCodecContext *avctx, const AVPacket *avpkt)
{
    ...
    if (!avci->buffer_frame->buf[0] && !dc->draining_started &&
        avctx->codec->type != AVMEDIA_TYPE_VIDEO) {
